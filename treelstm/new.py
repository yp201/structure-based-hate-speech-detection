...
def main():
    ...
    # trainer.optimizer.load_state_dict(checkpoint['optim'].state_dict)   
    best = -float('inf')
    ...
    for epoch in range(args.epochs):
        train_loss = trainer.train(training_generator)
        train_loss, train_pred = trainer.test(train_dataset)
        dev_loss, dev_pred = trainer.test(dev_dataset)
        test_loss, test_pred = trainer.test(test_dataset)
        train_accuracy= metrics.accuracy(train_pred,train_dataset.labels)
        train_pearson = metrics.pearson(train_pred, train_dataset.labels)
        train_mse = metrics.mse(train_pred, train_dataset.labels)

        logger.info('==> Epoch {}, Train \tLoss: {} \tAccuracy: {} \tPearson: {}\tMSE: {}'.format(
            epoch, train_loss,train_accuracy, train_pearson, train_mse))
        dev_pearson = metrics.pearson(dev_pred, dev_dataset.labels)
        dev_mse = metrics.mse(dev_pred, dev_dataset.labels)
        dev_accuracy=metrics.accuracy(dev_pred, dev_dataset.labels)
        logger.info('==> Epoch {}, Dev \tLoss: {}\tAccuracy: {}\tPearson: {}\tMSE: {}'.format(
            epoch, dev_loss,dev_accuracy, dev_pearson, dev_mse))
        test_pearson = metrics.pearson(test_pred, test_dataset.labels)
        test_mse = metrics.mse(test_pred, test_dataset.labels)
        test_accuracy = metrics.accuracy(test_pred, test_dataset.labels)

        logger.info('==> Epoch {}, Test \tLoss: {}\tAccuracy: {}\tPearson: {}\tMSE: {}'.format(
            epoch, test_loss,test_accuracy, test_pearson, test_mse))
